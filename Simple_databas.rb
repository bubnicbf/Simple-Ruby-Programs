#Write a simple tool to track a small set of data. The tool should have a commandline interface to 
#  enter at least two different values. The entered data should be stored in a structured format and saved to disk.
...
You should track the following details:
  ...
  - A date (either the date when the entry was made or some other date that is meaningful, like the birthday); 
      the date may be generated or entered manually
...
#The file format on disk should be human readable, but it need not be standardized. A natively available 
#  format that doesn't need an external library is preferred. Avoid developing your own format however if 
#  you can use an already existing one. If there is no existing format available pick one of: JSON, 
#  S-Expressions, YAML, or others.
